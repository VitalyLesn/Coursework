...
{
    // ПРИМЕЧАНИЕ. Команду "Переименовать" в меню "Рефакторинг" можно использовать для одновременного изменения имени класса "BugTrackerREST" в коде, SVC-файле и файле конфигурации.
    // ПРИМЕЧАНИЕ. Чтобы запустить клиент проверки WCF для тестирования службы, выберите элементы BugTrackerREST.svc или BugTrackerREST.svc.cs в обозревателе решений и начните отладку.
    public class BugTrackerREST : IBugTrackerREST
    {
        ...
        //Departmens
        public string InsertDepartmens(string name, string phone)
        {
            command.CommandText = "INSERT INTO Departments VALUES (@NameDep,@PhoneDep)";
            command.Parameters.AddWithValue("NameDep", name);
            command.Parameters.AddWithValue("PhoneDep", Convert.ToInt32(phone));
            command.CommandType = CommandType.Text;
            dbConnection.Open();
            return (command.ExecuteNonQuery() > 0 ? "Успешно" : "Ошибка");
        }
        public string UpdateDepartmens(string id, string name, string phone)
        {
        ...
        public string DeleteDepartmens(string id)
        {
            dbConnection.Close();
            command = dbConnection.CreateCommand();
        ...
        //Bugs
        public string InsertBugs(string idproj, string infobug, string priority, string idempl, string status)
        {
            command.CommandText = @"INSERT INTO Bugs VALUES (@IDProj,  @InfoBug,  @Priority, @IDEmpl, @Status)";
            command.Parameters.AddWithValue("IDProj", Convert.ToInt32(idproj));
            command.Parameters.AddWithValue("InfoBug", infobug);
            command.Parameters.AddWithValue("Priority", priority);
            command.Parameters.AddWithValue("IDEmpl", Convert.ToInt32(idempl));
            command.Parameters.AddWithValue("Status", status);
            command.CommandType = CommandType.Text;
            dbConnection.Open();
            return (command.ExecuteNonQuery() > 0 ? "Успешно" : "Ошибка");
        }
        public string UpdateBugs(string idbug, string idproj, string infobug, string priority, string idempl, string status)
        {
            ...
            command.Parameters.AddWithValue("IDEmpl", Convert.ToInt32(idempl));
            command.Parameters.AddWithValue("Status", status);
            command.Parameters.AddWithValue("ID", Convert.ToInt32(idbug));
            command.CommandType = CommandType.Text;
            dbConnection.Open();
            return (command.ExecuteNonQuery() > 0 ? "Успешно" : "Ошибка");
        }
        ...
        public string DeleteBugs(string id)
        {
            command = dbConnection.CreateCommand();
        ...
        //Employees
        public string InsertEmployees(string fio, string login, string iddep, string position, string phoneempl, string email)
        {
            command.CommandText = "INSERT INTO Employees VALUES(@FIO, @Login, @IDDep, @Position, @PhoneEmpl, @E_mail)";
            command.Parameters.AddWithValue("FIO", fio);
            command.Parameters.AddWithValue("Login", login);
            command.Parameters.AddWithValue("IDDep", Convert.ToInt32(iddep));
        ...
        public string UpdateEmployees(string id, string fio, string login, string iddep, string position, string phoneempl, string email)
        {
        ...
        public string UpdateProject(string id, string nameproj, string abbrevproj, string defenproj)
        {
